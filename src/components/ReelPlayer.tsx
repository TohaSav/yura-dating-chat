    ...
    >
      ...
      {!isPlaying && !videoError && !isBuffering && (
        <div
          ...
          onClick={togglePlay}
        >
          <div className="w-20 h-20 bg-white/90 rounded-full flex items-center justify-center shadow-lg backdrop-blur-sm hover:bg-white transition-all duration-200">
      ...
      {/* Volume Control */}
      <div
        className={`absolute bottom-36 left-4 transition-opacity duration-300 ${showControls || !isPlaying ? "opacity-100" : "opacity-0"}`}
      >
        <Button
          size="sm"
          variant="ghost"
      ...
      {/* Views Counter */}
      <div className="absolute top-20 right-4 bg-black/60 text-white px-3 py-1 rounded-full text-xs backdrop-blur-sm border border-white/20">
